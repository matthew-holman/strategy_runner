...
def validate_signals_from_previous_trading_day() -> pd.DataFrame:
    ...
    # return only passing rows for downstream sizing (optional)
    return validated[validated["validated_at_open"]].copy()
...
def _persist_validation_results(sigs, validated_df: pd.DataFrame) -> None:
    # Build quick lookups
